...
def fetch_data_from_github() -> pd.DataFrame:
    ...
    # Matches your exact uploaded CSV structure
            ...
            if st.form_submit_button("Save Registration Metadata"):
                if f_name and woreda and kebele:
                    with st.spinner("Processing transaction package to cloud..."):
                        ...
                        # Maps directly to your exact custom headers
                        ...
                        if save_data_to_github(updated_df):
                            st.success(f"✅ Sync Successful! Record logged to GitHub for {f_name}.")
                        else:
                            st.error("❌ Cloud transaction rejected. Verify token repo permissions or repository casing strings.")
                else:
                    st.error("Name, Woreda, and Kebele data fields are mandatory.")

# ============================================================================
# INTERFACE: ADMINISTRATIVE PANELS
# ============================================================================
    ...
    if not st.session_state["auth"]:
        ...
        with st.expander("🔍 Cloud Database Connection Diagnostic Tester"):
            if st.button("Execute Verification Framework"):
                ...
                if res.status_code == 200:
                    st.success("✅ Connectivity & Read Access: SUCCESS")
                    scopes = res.headers.get("X-OAuth-Scopes", "")
                    if "repo" in scopes or "public_repo" in scopes:
                        st.success(f"✅ Secure Write Access: SUCCESS (Scopes: {scopes})")
                    else:
                        st.error(f"❌ Secure Write Access: FAILED (Token missing repository write permissions. Scopes found: {scopes})")
                else:
                    st.error(f"❌ Sync Failure: Target not found (Status Code: {res.status_code}). Check spelling or token repo access.")
        ...
        has_records = len(df) > 0 if not df.empty else False
        ...
        if has_records:
            ...
            with st.spinner("Decoding audio binary streams..."):
                z_buf = BytesIO()
            ...
            if st.button("PERMANENTLY FLUSH CLOUD REPOSITORY RECORDS", type="primary", use_container_width=True):
                ...
                if save_data_to_github(empty_df):
                    st.success("Cloud spreadsheets successfully wiped.")
                    st.rerun()
        else:
            st.info("No surveyor records are currently stored inside your remote GitHub cloud database file.")rt streamlit as st
import pandas as pd
import base64
import zipfile
import datetime
...
def fetch_data_from_github() -> pd.DataFrame:
    ...
    # Return empty structured dataframe if file doesn't exist yet
            ...
            if st.form_submit_button("Save Registration Metadata"):
                if f_name and woreda and kebele:
                    with st.spinner("Processing transaction package to cloud..."):
                        ...
                        try:
                            next_id = int(pd.to_numeric(df["id"]).max() + 1) if not df.empty else 1
                        except:
                            next_id = len(df) + 1
                        
                        # 3. Compile new entry vector
                        new_entry = pd.DataFrame([{
                            "id": next_id,
                            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            "name": f_name,
    ...
    if not st.session_state["auth"]:
        ...
        if has_records:
            ...
            with st.spinner("Decoding audio binary streams from database..."):
                ...
                with zipfile.ZipFile(z_buf, "w") as zf:
                    for idx, row in df.iterrows():
                        audio_str = row.get('audio_data', '')
                        if pd.notna(audio_str) and str(audio_str).strip() != "":
                            try:
                                binary_audio = base64.b64decode(audio_str)
                                farmer_name = str(row.get('name', f'Unknown_{idx}')).replace(" ", "_")
                                zf.writestr(f"ID_{row.get('id', idx)}_{farmer_name}.mp3", binary_audio)
                            except:
                                pass
            c2.download_button("🎤 Extract Voice Recordings Archive (ZIP)", z_buf.getvalue(), "Amhara_ME_Audios.zip", use_container_width=True)
            ...
            if st.button("PERMANENTLY FLUSH CLOUD REPOSITORY RECORDS", type="primary", use_container_width=True):
                empty_df = pd.DataFrame(columns=["id", "timestamp", "name", "woreda", "kebele", "phone", "audio_data", "registered_by"])
                if save_data_to_github(empty_df):
